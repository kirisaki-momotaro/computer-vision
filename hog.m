...
for i = 1:nori
  ...
  B = (I>thresh)&(ori+90>(i-1)*20)&(ori+90<(i-1)*20+20);
  %im2bw(I,thresh)& im2bw(ori+90,(nori-1)*20)& not(im2bw(ori+90,(nori-1)*20+20));
...
%ohist=ohist./sum(ohist,3);
%NORMALISATION

ohist_sum=zeros(size(ohist(:,:,2)));
for j=1:nori
    ohist_sum=ohist_sum+ohist(:,:,j);
end
for j=1:nori
    ohist(:,:,j)=ohist(:,:,j)./ohist_sum;
end
ohist(isnan(ohist))=0;
